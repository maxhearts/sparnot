#!/usr/bin/env python3
"""Compile a fixture JSON file to IR bundle."""

import sys
import json
from pathlib import Path

from sparnot.schemas import NarrativeIntent
from sparnot.poc_compiler.models import (
    NarrativeIR,
    CharacterIR,
    ArcIR,
    Proposed,
...
from sparnot.poc_compiler import rules_tables
from sparnot.poc_compiler.hashing import compute_all_hashes


def compile_narrative_intent(narrative_intent: NarrativeIntent, characters=None, arc=None):
    """Compile schemas to IR bundle."""
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python compile_fixture.py <fixture_name> [output_path]")
        print("\nAvailable fixtures:")
        fixtures_dir = Path(__file__).parent / "sparnot" / "poc_compiler" / "fixtures"
    ...
    if not fixture_path.exists():
        print(f"✗ Fixture '{fixture_name}' not found in {fixtures_dir}")
        sys.exit(1)
    
    # Load narrative intent
    print(f"Loading fixture: {fixture_path}")
    try:
        with open(fixture_path) as f:
            fixture_data = json.load(f)
        narrative_intent = NarrativeIntent(**fixture_data)
    except Exception as e:
        print(f"✗ Failed to load narrative intent: {e}")
        sys.exit(1)
    
    # Compile
    print("Compiling...")
    try:
        bundle = compile_narrative_intent(narrative_intent)
    except Exception as e:
    ...
    if output_path:
        ...
        output_file = fixtures_dir / f"{fixture_name}_compiled.json"
    
    # Save bundle
    print(f"Saving bundle to: {output_file}")
    with open(output_file, "w") as f:
        json.dump(bundle.model_dump(mode="json"), f, indent=2)
    
    # Show summary
    print(f"\n✓ Compiled bundle saved to: {output_file}")
